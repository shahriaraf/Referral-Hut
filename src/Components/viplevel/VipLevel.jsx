        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
            <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-4 pt-4"><button type="submit" disabled={loading} className="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded-md disabled:opacity-50 flex items-center justify-center gap-2">{loading ? <div className="animate-spin rounded-full h-5 w-5 border-b-2"></div> : <FaSave />} {loading ? 'Updating...' : 'Update'}</button><button type="button" onClick={onClose} className="px-4 py-2 bg-gray-600 hover:bg-gray-700 rounded-md">Cancel</button></div>
                </form>
            </div>
        </div>
    );
...
const AdminVIPLevels = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto p-6">
            <h2 className="text-3xl font-bold text-amber-400 mb-8 text-center">Admin - VIP Program Levels</h2>
                ...
                {levels.map((level) => (
                    <div key={level.level} className="bg-gray-800 rounded-lg p-6 shadow-lg border border-gray-700">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button onClick={() => handleEditClick(level)} className="p-2 bg-amber-600 hover:bg-amber-700 text-white rounded-full"><FaEdit /></button>
                        </div>
